...
// @ts-ignore;
import { Upload, FileText, Download, ArrowLeft, CheckCircle, AlertCircle, Loader2 } from 'lucide-react';
...
export default function Document(props) {
  ...
  const uploadFile = async fileObj => {
    ...
    try {
      ...
      toast({
        title: "处理成功",
        description: `文档 ${fileObj.name} 已成功处理完成`
      });
    } catch (error) {
      setFiles(prev => prev.map(f => f.id === fileObj.id ? {
        ...f,
      ...
      toast({
        title: "处理失败",
        description: `文档 ${fileObj.name} 处理失败：${error.message}`,
        variant: "destructive"
      });
    } finally {
      setUploading(false);
  ...
  return <div style={style} className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100">
          ...
          pageId: 'home'
              ...
              <div>
                <h1 className="text-3xl font-bold text-gray-800">文档处理</h1>
                <p className="text-gray-600">PDF、Word、Excel等文档格式转换、合并、压缩</p>
              </div>
            </div>
              ...
              <CardContent>
                <div className={`border-2 border-dashed rounded-lg p-8 text-center transition-colors ${dragActive ? 'border-blue-500 bg-blue-50' : 'border-gray-300 hover:border-gray-400'}`} onDragEnter={handleDrag} onDragLeave={handleDrag} onDragOver={handleDrag} onDrop={handleDrop}>
                  ...
                  <p className="text-sm text-gray-500 mb-4">
                    支持 PDF、Word、Excel、PPT 等格式，最大文件大小 100MB
                  </p>
                  <Button onClick={() => fileInputRef.current?.click()} disabled={uploading}>
                    {uploading ? '处理中...' : '选择文档'}
                  </Button>
                  <input ref={fileInputRef} type="file" multiple accept=".pdf,.doc,.docx,.xls,.xlsx,.ppt,.pptx" onChange={handleFileInput} className="hidden" />
                </div>
              </CardContent>
            </Card>
